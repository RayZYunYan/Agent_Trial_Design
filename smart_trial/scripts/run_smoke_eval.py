"""Clean smoke test: wipe old eval test outputs, run 1 baseline + 1×9 grid, validate."""
...
# Legacy pilot / partial grid outputs to remove before a clean run.
DIRS_TO_WIPE = [
    PROJECT_ROOT / "smart_trial" / "outputs" / "eval" / "pilot",
    PROJECT_ROOT / "smart_trial" / "outputs" / "eval" / "grid",
    PROJECT_ROOT / "smart_trial" / "outputs" / "eval" / "baseline",
    PROJECT_ROOT / "smart_trial" / "outputs" / "eval" / "_test_summary_tmp",
]
...
def _wipe_dir(path: Path) -> None:
    """Best-effort remove directory (handles open files on Windows)."""
    if not path.exists():
        return
...
def main() -> int:
    ...
    print("=== Removing old test output directories ===")
    ...
    if use_groq:
        ...
        print("\nMode: Groq patient/doctor + mock judge")
    else:
        env["SMART_TRIAL_USE_MOCK"] = "1"
        print("\nMode: full mock (set GROQ_API_KEY in .env for Groq dialogue)")

    py = sys.executable
    _run([py, "-m", "smart_trial.run_eval", "--config", "smart_trial/config/eval/config_smoke_baseline.yaml", "--no-resume"], env)
    _run([py, "-m", "smart_trial.run_eval", "--config", "smart_trial/config/eval/config_smoke_grid.yaml", "--no-resume"], env)
    ...
    if grid_report["ok"] and len(baseline_rows) == 1 and category_ok:
        print("\n=== SMOKE OK ===")
        return 0
