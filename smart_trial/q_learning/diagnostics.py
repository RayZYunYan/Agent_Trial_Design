...
def sample_size_report(df: pd.DataFrame, result: Optional[QLearningResult] = None) -> pd.DataFrame:
    ...
    arm_cell = df.groupby(["A1", "A2", "A3"], dropna=False).size().reset_index(name="count")
    ...
    if result is not None and n < result.stage3.n_parameters:
...
def arm_cell_table(df: pd.DataFrame) -> pd.DataFrame:
    """Cross-tab of observed (A1, A2, A3) with outcome rate."""
    g = df.groupby(["A1", "A2", "A3"], dropna=False).agg(
        n=("Y", "size"),
        mean_Y=("Y", "mean"),
    ).reset_index()
