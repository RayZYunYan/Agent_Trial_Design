...
def compare_rules(q_rules: pd.DataFrame, a_rules: pd.DataFrame) -> pd.DataFrame:
    """Row-by-row agreement on (π̂1, π̂2). Requires same encounter_id order."""
    merged = q_rules[["encounter_id", "pi1", "pi2"]].merge(
        a_rules[["encounter_id", "pi1", "pi2"]],
        on="encounter_id",
        suffixes=("_q", "_a"),
    )
    merged["agree_pi1"] = merged["pi1_q"] == merged["pi1_a"]
    merged["agree_pi2"] = merged["pi2_q"] == merged["pi2_a"]
    merged["agree_all"] = merged[["agree_pi1", "agree_pi2"]].all(axis=1)
    return merged
